...
namespace EIBFormDesigner.Parse
{
    ...
    public class Scanner
    {
        ...
        public Scanner(string textToParse)
        {
            try
            {
                Stream stream = new MemoryStream(ASCIIEncoding.Default.GetBytes(textToParse));
                buffer = new Buffer(stream, false);
                Init();
            }
            catch (IOException)
            {
                throw new FatalError("Cannot parse text" + textToParse);
            }
        }
