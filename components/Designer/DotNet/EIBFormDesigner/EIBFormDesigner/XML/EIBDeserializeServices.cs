...
namespace EIBFormDesigner.XML
{
    class EIBDeserializeServices
    {
            ...
            foreach (XmlNode xmlChildNode in xmlNode.ChildNodes)
            {
                if (xmlChildNode.NodeType == XmlNodeType.Element)
                {
                    ...
                    else if (windowName.Equals(FormDesignerConstants.FormPattern))
                    {
                        ...
                        {
                            FormDesignerUtilities.formWindowNames.Add(newChildNode.Text, newChildNode.Text.ToLower());
                        }
        ...
        public static void DeserializeWindowXML(string fileName, Control baseFrame, XmlNode parentXMLNode)
        {
                ...
                foreach (XmlNode xmlNode in currentXmlNode.ChildNodes)
                {
                    ...
                    if (xmlNode.NodeType == XmlNodeType.Element)
                    {
                        ...
                        if (xmlNode.Name == FormDesignerConstants.TextBoxControl)
                        {
                            ...
                            EIBTextBox.counter++;
                            //FormDesignerUtilities.formWindowNames.Add(xmlNode.Attributes[XMLServicesConstants.XmlNodeIdAtt].Value, xmlNode.Attributes[XMLServicesConstants.XmlNodeIdAtt].Value.ToLower());
                            ControlValidation.addUniqueIdToList(xmlNode.Attributes[XMLServicesConstants.XmlNodeIdAtt].Value);
                        }
                        ...
                        if (xmlNode.Name.Trim().Equals(FormDesignerConstants.ScrollableRowControl, StringComparison.CurrentCultureIgnoreCase))
                        {
                            EIBScrollableRow newNode = new EIBScrollableRow();
                        ...
                        if (xmlNode.Name.Trim().Equals(FormDesignerConstants.ColumnControl, StringComparison.CurrentCultureIgnoreCase))
                        {
                            ...
                            if (currentControl is EIBScrollableRow)
                            {
                                EIBScrollableRow row = (EIBScrollableRow)currentControl;
            ...
            {
                String errStr = "Error occured in file " + xmlFileName + "\r\n";
                ...
                foreach (XmlNode Node in menuNode.ChildNodes)
                {
                    if (Node.Name == FormDesignerConstants.MenuControl)
                    {
                       ...
                       // submenu.InitiateSettings(null);
                        ...
                        submenu.ControlName = Node.Name;
                        submenu.UniqueId = Node.Attributes["id"].Value;
                        if (menuBar is EIBMenuBar)
                        {
                        ...
                        EIBMenu.counter++;
                      //  FormDesignerUtilities.formWindowNames.Add(submenu.UniqueId.ToString(), submenu.UniqueId.ToString().ToLower());
                        ControlValidation.addUniqueIdToList(submenu.UniqueId.ToString());
                    }
                    else if (Node.Name == FormDesignerConstants.MenuItemControl)
                    {
                       ...
                       // menuItem.InitiateSettings(null);
                        menuItem.Click += new EventHandler(FormDesigner.eventManager.handleMenuItemClick);
                        menuItem.ControlName = Node.Name;
                        menuItem.UniqueId = Node.Attributes["id"].Value;
                        if (menuBar is EIBMenuBar)
                        {
                        ...
                        EIBMenuItem.counter++;
                      //  FormDesignerUtilities.formWindowNames.Add(menuItem.UniqueId.ToString(), menuItem.UniqueId.ToString().ToLower());
                        ControlValidation.addUniqueIdToList(menuItem.UniqueId.ToString());
                    }
                }
                if (menuBar is EIBMenuBar)
                {
        ...
        private static void SetMenuValue(ToolStripItem control, XmlNode xmlNode)
        {
            ...
            if (attributeCount > 0)
            {
                for (int counter = 0; counter < attributeCount; counter++)
                {
                    ...
                    if (control is EIBMenu)
                    {
                        if (xmlNode.Attributes[counter].Name == XMLServicesConstants.XmlNodeRenderAsMenuItemAtt)
                        {
                            ((EIBMenu)control).RenderAsMenuItem = xmlNode.Attributes[counter].Value.ToLower().Equals("true");
                        }
                    }
                }
            }
        ...
        private static void SetAttributeValue(Control control, string propertyName, string value)
        {
            try
            {
                if (control is EIBColumn)
                {
                    ...
                    else if (propertyName == XMLServicesConstants.XmlNodeSortAscendingAtt)
                    {
                        ((EIBColumn)control).SortAscending = value;
                    }

                    else if (propertyName == XMLServicesConstants.XmlNodeSortDescendingAtt)
                    {
                        ((EIBColumn)control).SortDescending = value;
                    }
                }
                ...
                if (propertyName == XMLServicesConstants.XmlNodeHeightTypeAtt)
                {
                    if (control is ICustomSizableControl)
                    {
                        if (control is EIBTabControl)
                        {
                            if (value == HeightMeasurement.Auto.ToString())
                            {
                                ((EIBTabControl)control).HeightType = HeightMeasurement.Auto;
                            }
                            else if (value == HeightMeasurement.Pixel.ToString())
                            {
                                ((EIBTabControl)control).HeightType = HeightMeasurement.Pixel;
                            }
                            else if (value == HeightMeasurement.Percent.ToString())
                            {
                                ((EIBTabControl)control).HeightType = HeightMeasurement.Percent;
                            }
                        }
                    }
                }
                ...
                {
                    if ((control is ICustomSizableControl) && (control is EIBTabControl))
                    {
                        ((ICustomSizableControl)control)._Height = Int32.Parse(value);
                    }
                    else
                    {
                ...
                if (propertyName == XMLServicesConstants.XmlNodeSourceAtt)
                {
                    ...
                    //    //ms.Close();
                    //    //ms.Dispose();
                    //    File.Copy(panel.Source + @"\" + value, fileNameValue, true);
                    //}
                    GC.Collect();
                ...
                if (propertyName == XMLServicesConstants.XmlNodeBackgroundImageAtt)
                {
                    ...
                    { ((EIBLabel)control).Image = imgInFile; }
                ...
                if (propertyName == XMLServicesConstants.XmlNodeMultiLineAtt)
                {
                    ...
                    else if (control is EIBLabel)
                    {
                        ...
                        label.Multiline = value.ToLower().Equals("true");
                    }
                }
                ...
                if (propertyName == XMLServicesConstants.XmlNodeTextAlignAtt)
                {
                    ...
                    {
                        EIBButton button = (EIBButton)control;
                        button.TextAlign = TextAlignment(value);
                    }
                }
                ...
                if (propertyName == XMLServicesConstants.XmlNodeBorderSizeAtt)
                {
                    ...
                    {
                        ((EIBTextBox)control).BorderSize = int.Parse(value);
                    }
                    else if(control is EIBGrid)
                    {
                        ((EIBGrid)control).BorderSize = int.Parse(value);
                    }
                }
                if (propertyName == XMLServicesConstants.XmlNodeBorderColorAtt)
                {
                    if (control is EIBTextBox)
                    {
                        if (!value.Contains("#"))
                        {
                            ((EIBTextBox)control).BorderColor = Color.FromName(value);
                        }
                        else
                        {
                            ((EIBTextBox)control).BorderColor = ColorTranslator.FromHtml(value);
                        }
                    }
                    else if (control is EIBGrid)
                    {
                        if (!value.Contains("#"))
                        {
                            ((EIBGrid)control).BorderColor = Color.FromName(value);
                        }
                        else
                        {
                            ((EIBGrid)control).BorderColor = ColorTranslator.FromHtml(value);
                        }
                    }
                }
                if (propertyName == XMLServicesConstants.XmlNodeRenderCursorAtt)
                {
                    if (control is EIBButton)
                    {
                        ((EIBButton)control).RenderCursor = value.ToLower().Equals("true");
                    }
                    else if (control is EIBLabel)
                    {
                        ((EIBLabel)control).RenderCursor = value.ToLower().Equals("true");
                    }
                }
                if (propertyName == XMLServicesConstants.XmlNodeRenderTextWithImgAtt)
                {
                    ((EIBButton)control).RenderTextWithImage = value.ToLower().Equals("true");
                }
                ...
                if (propertyName == XMLServicesConstants.XmlNodeMinLengthAtt)
                {
                    if (control is EIBTextBox)
                    {
                        EIBTextBox textBox = (EIBTextBox)control;
                        textBox.MinLength = Int32.Parse(value);
                    }
                }
                //if (propertyName == XMLServicesConstants.XmlNodeConstraintAtt)
                //{
                //    if (control is EIBTextBox)
                //    {
                //        EIBTextBox textBox = (EIBTextBox)control;
                //        textBox.Constraint = value;
                //    }
                //}
            ...
            {
                String errStr = "Error-SetAttributeValue: " + ex.Message + "\r\n";
                errStr += "ControlName: " + control.Name + "\r\n";
                errStr += "Attribute: " + propertyName + "\r\n";
                errStr += "Value: " + value;
                MessageBox.Show(errStr);
                throw ex;
            }
        ...
        private static FontStyle GetFontStyle(string value)
        {
            ...
            if (value == "Bold, Underline")
            {
                fontStyle = System.Drawing.FontStyle.Bold | System.Drawing.FontStyle.Underline;
            }
            if (value == "Bold, Italic, Underline")
            {
                fontStyle = System.Drawing.FontStyle.Bold | System.Drawing.FontStyle.Italic | System.Drawing.FontStyle.Underline;
            }
            if (value == "Italic, Underline")
            {
                fontStyle = System.Drawing.FontStyle.Italic | System.Drawing.FontStyle.Underline;
            }
            return fontStyle;
        }
