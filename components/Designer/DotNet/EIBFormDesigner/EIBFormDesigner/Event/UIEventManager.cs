    ...
    public class UIEventManager : IUIEventManager
    {
        ...
        public void handleDrag(object sender, EventArgs e, object panel, Point NewLocation)
        {
            ...
            if (form.currentBaseWindow.TypeOfWindow == FormDesignerConstants.FormPattern)
            {
                ...
                if (text.Trim().Equals(FormDesignerConstants.ScrollableRowControl))
                {
                    ctrl = (EIBScrollableRow)toolBox.getControlType(text.Trim());
                    if (currentControl[0] is EIBGrid)
                    {
                        EIBGrid tablePanel = (EIBGrid)currentControl[0];
                        //tablePanel.SuspendLayout();
                        EIBScrollableRow rowTable = (EIBScrollableRow)ctrl;
                ...
                if (text.Trim().Equals(FormDesignerConstants.ColumnControl))
                {
                    ...
                    if (currentControl[0] is EIBScrollableRow && ((EIBScrollableRow)currentControl[0]).Parent is EIBGrid)
                    {
                        ...
                        EIBScrollableRow rowTable = (EIBScrollableRow)currentControl[0];
            ...
            if (ctrl != null)
            {
                ...
                else if (panel is EIBGrid)
                {
                    EIBGrid grid = (EIBGrid)panel;
                    try
                    {
                ...
                else if (panel is EIBScrollableRow)
                {
                    EIBScrollableRow row = (EIBScrollableRow)panel;
                ...
                if (!(ctrl is EIBMenuBar  || ctrl is EIBColumn))
                {
        ...
        public void addPropertiesControl(Control ctrl)
        {
            try
            {
                ...
                if (!(ctrl is EIBTabPage) && !(ctrl.Parent is EIBGrid) && !(ctrl.Parent is EIBLattice) &&
                     !(ctrl.Parent is EIBColumn) &&
                    !(ctrl is EIBColumn)  && !(ctrl is EIBFormDesigner.Workflow.Controls.MarkControl))
                {
                    if (pickBox.ContainsKey((IEIBControl)ctrl))
        ...
        public void handleControlClick(object sender, MouseEventArgs e)
        {
            try
            {
                ...
                if (!((ctrl.Parent is EIBLattice) || (ctrl.Parent is EIBGrid) ))
                {
        ...
        public void TableControl_Layout(object sender, LayoutEventArgs e)
        {
            try
            {
                ...
                foreach (EIBScrollableRow row in grid.Rows)
                {
                    if (row.Visible)
                    {
                        count++;
        ...
        public void LatticeControl_Layout(object sender, LayoutEventArgs e)
        {
            try
            {
                ...
                foreach (EIBScrollableRow row in lattice.Rows)
                {
                    if (row.Visible)
                    {
                        count++;
